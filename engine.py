...
class VectorEngine:
    ...
    def search(self, query, limit=20):
        ...
        if self.config.ENABLE_QUERY_BOOSTING:
            ...
            if boosts:
                ...
                # Calculate max possible boost for calibration
                max_possible_boost = sum(b.get("boost", 0.05) for b in boosts)

                # Calibrate: cap total boost at 40% of max similarity
                # This prevents boosts from overwhelming semantic ranking
                boost_cap = max_similarity * 0.4

                for i, song_id in enumerate(self.ids):
                    song = songs_map.get(song_id)
                    if song:
                        bonus = self._get_song_boost(song, boosts)
                        # Apply cap to prevent boost from dominating
                        capped_bonus = min(bonus, boost_cap)
                        similarities[i] += capped_bonus
    ...
    def _get_song_boost(self, song, boosts):
        ...
        for boost in boosts:
            ...
            # Check lyrics emotion constraints
