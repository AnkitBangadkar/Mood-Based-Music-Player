...
def _clean_text(text):
    ...
    # Remove text in brackets (but keep content inside for titles like "Song (Remix)")
    text = re.sub(
        r"\((?:Official\s*)?(?:Audio|Video|Lyric\s*Video|Visualizer|Music\s*Video|HD|HQ|4K)\)",
        "",
        text,
        flags=re.IGNORECASE,
    )
    ...
    # Remove specific keywords (and everything after them)
    text = re.sub(r"(?i)\b(?:feat\.|ft\.|featuring|cv:|ost).*", "", text)
